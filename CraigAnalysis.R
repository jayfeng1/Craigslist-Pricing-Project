craig = read.csv("craig.csv")
craig$zipcode = as.factor(craig$zipcode)
#Some basic example models
...
craigLm1 = lm(price ~ beds + size + zipcode + numPic + baths, data = craig)
craigLm2 = lm(price ~ beds + size + zipcode, data = craig)

#Price is 2700
predict(craigLm1, newdata = data.frame(beds = 2, size = 1200, contentLen = 5, numPic = 7, baths = 2, zipcode = "98101"), type='response')
#Price is 4500
...
listNum = (tail(sort(table(craig$zipcode)), 40))
craigClean = craig[craig$zipcode %in% row.names(listNum),]
